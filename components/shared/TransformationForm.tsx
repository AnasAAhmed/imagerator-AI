...
const TransformationForm = ({ action, data = null, userId, type, creditBalance, config = null }: TransformationFormProps) => {
  ...
  const [isTransformating, setIsTransformating] = useState(false);
      ...
      if (action === 'Update') {
        try {
          const updatedImage = await updateImage({
            ...
            userId, path: `/transformations/${data._id}`
          })
          if (updatedImage) {
            form.reset()
            setImage(data);
            ...
            {(type === 'remove' || type === 'recolor') && (
              <div className="prompt-field">
                <CustomField
                  ...
                  className="w-full"
                  render={({ field }) => <Input value={field.value} className="input-field"
